...
def image_to_world(cx, cy, robot, target_color, q1, frame_width=1296, frame_height=972):
    """Chuyển đổi tọa độ ảnh sang tọa độ thực tế (mm) với gốc ở giữa dưới đáy."""
    z_heights = {"red": 40, "green": 40, "blue": 40}  # Độ cao giả định của vật thể
    z_height = z_heights.get(target_color, 40)
    
    # Vị trí camera (tâm ảnh: 648, 486) khi q1=0°, q2=90°, q3=-90°
    camera_offset = np.array([90, 0, 110])  # [x, y, z] - Tâm camera cách gốc robot (mm)
    ...
    camera_position = np.dot(R_q1, camera_offset)  # Vị trí tâm camera trong hệ robot
    ...
    # Gốc tọa độ ảnh mới: giữa dưới đáy (648, 972)
    pixel_origin_x = frame_width / 2  # 648
    pixel_origin_y = frame_height     # 972
    
    # Tính góc từ tâm camera (648, 486) đến vật thể (cx, cy)
    pixel_center_x = frame_width / 2   # 648
    pixel_center_y = frame_height / 2  # 486
    angle_x_total = np.radians((cy - pixel_center_y) * (fov_vertical / frame_height))  # Góc dọc từ tâm
    angle_y = np.radians((pixel_center_x - cx) * (fov_horizontal / frame_width))      # Góc ngang từ tâm
    
    # Độ cao từ camera đến mặt phẳng vật thể
    camera_height = camera_position[2] - z_height
    if camera_height <= 0:
        camera_height = 1  # Tránh chia cho 0
    
    # Tọa độ từ tâm camera đến vật thể trong hệ camera
    x_camera_total = camera_height * np.tan(angle_x_total)  # Khoảng cách dọc từ tâm
    y_camera = camera_height * np.tan(angle_y)              # Khoảng cách ngang (trái dương)
    
    # Khoảng cách từ tâm ảnh (486) đến gốc mới (972) trong ảnh
    offset_pixels_y = pixel_origin_y - pixel_center_y  # 972 - 486 = 486 pixels
    angle_x_offset = np.radians(offset_pixels_y * (fov_vertical / frame_height))
    x_offset = camera_height * np.tan(angle_x_offset)  # Khoảng cách thực tế từ tâm đến gốc mới
    
    # Tọa độ vật thể so với gốc mới (648, 972)
    x_camera = x_camera_total - x_offset  # Trục x (dọc, từ dưới lên)
    
    # Tọa độ trong hệ camera (z=0 tại mặt phẳng vật thể)
    P_camera = np.array([x_camera, y_camera, 0])
    
    # Ma trận quay của camera (camera hướng xuống, x lên, y trái dương)
    ...
    print(f"Angle x total: {np.degrees(angle_x_total):.2f}°, Angle y: {np.degrees(angle_y):.2f}°")
    print(f"Offset x (tâm đến gốc): {x_offset:.2f} mm")
